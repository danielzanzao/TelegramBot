...
from data_models import BotData, storage, WorkHour, Reminder, ReminderFrequency, ReminderType, MemberProfile
        ...
        if scopes:
            ...
            if remaining_scopes:
                ...
                for s in remaining_scopes:
                    ...
                    prefix = "Gerente do" if s in ["Odoyá", "Maná"] else "Gerente de"
...
Selecione uma das opções abaixo para começar:
        ...
        is_gp_role = BotData.is_user_in_role(user.id, "GP")
        ...
        debug_roles = BotData.get_user_role(user.id)

        # welcome_text += f"\n\n🔍 Debug: Roles={debug_roles}, Scopes={managed_scopes}, IsGP={is_gp}"
        ...
        if not roles:
             ...
             # User not registered or has no roles

             await update.message.reply_text(

                 f"🌟 Olá {user.first_name}! \n\n"

                 "Parece que seu cadastro ainda não foi encontrado no meu banco de dados.\n"

                 "Por favor, utilize o comando /cadastro para se registrar e começar a usar o bot. 👇",
    ...
    async def _start_reassign_flow(self, message, session):

        """Start the reassignment flow (GP Only)"""

        user_id = message.from_user.id

        

        # Check permission (Double check, although button is hidden)

        if not BotData.is_user_in_role(user_id, "GP"):

             await message.reply_text("❌ Acesso negado. Apenas membros de GP podem acessar essa função.")
        ...
        session['step'] = 'reassign_member'

        session['waiting_for'] = 'reassign_member'

        

        # List all members for selection

        # Create keyboard with all member names

        all_members = sorted([m.name for m in BotData.MEMBERS_DB])
        ...
        for i in range(0, len(all_members), 2):
            ...
            row.append(KeyboardButton(all_members[i]))

            if i + 1 < len(all_members):

                row.append(KeyboardButton(all_members[i+1]))

            keyboard.append(row)

            

        # Add Cancel option? Or Back?
        ...
        await message.reply_text(
            ...
            "Selecione o **Membro** que você deseja remanejar:",
    ...
    async def _handle_reassign_member_selection(self, update, session, text):

        """Handle member selection for reassignment"""

        user_input = text.strip()

        all_members = BotData.MEMBERS_DB

        candidate_names = [m.name for m in all_members]

        

        matches = get_close_matches(user_input, candidate_names, n=1, cutoff=0.6)

        

        if not matches:

             await update.message.reply_text("❌ Membro não encontrado. Tente novamente.")
        ...
        target_member = next((m for m in all_members if m.name == target_name), None)

        

        if not target_member:

             await update.message.reply_text("❌ Erro ao recuperar dados do membro.")

             return

             

        session['reassign_target_name'] = target_name

        session['reassign_target_id'] = target_member.telegram_id

        session['reassign_target_current_roles'] = target_member.roles

        session['reassign_target_scopes'] = target_member.managed_scopes

        

        # Check Permissions Logic

        # 1. Accessor must be GP (Checked at start)

        # 2. Check if Accessor is Manager

        accessor_scopes = BotData.get_managed_scopes(update.effective_user.id)

        is_gp_manager = "GP" in accessor_scopes

        

        session['is_gp_manager'] = is_gp_manager

        

        # Restriction: Non-managers cannot touch Leadership (ManageScope > 0)

        target_is_leadership = len(target_member.managed_scopes) > 0

        

        if not is_gp_manager and target_is_leadership:

             await update.message.reply_text(

                 f"❌ Permissão Negada.\n"

                 f"O membro **{target_name}** possui cargo de liderança (Diretor/Gerente).\n"

                 "Apenas Gerentes de GP podem alterar cargos de liderança."
        ...
        # Ask for new Role Type

        keyboard = [

            [KeyboardButton("👤 Membro")],
        ...
        if is_gp_manager:
            ...
            keyboard.append([KeyboardButton("💼 Gerente"), KeyboardButton("👑 Diretor")])
        ...
        await update.message.reply_text(
            ...
            f"👤 Membro selecionado: **{target_name}**\n"

            f"ℹ️ Cargos atuais: {', '.join(target_member.roles)}\n"

            f"ℹ️ Liderança: {', '.join(target_member.managed_scopes) if target_member.managed_scopes else 'Nenhuma'}\n\n"

            "Selecione o **Novo Nível** para este membro:",
        ...
        session['step'] = 'reassign_role_type'

        session['waiting_for'] = 'reassign_role_type'



    async def _handle_reassign_role_type(self, update, session, text):

        """Handle role type selection"""

        if text not in ["👤 Membro", "💼 Gerente", "👑 Diretor"]:
        ...
        session['reassign_new_type'] = text

        

        # Access Check again for safety (though keyboard limits it)

        if text in ["💼 Gerente", "👑 Diretor"] and not session.get('is_gp_manager'):

             await update.message.reply_text("❌ Apenas Gerentes de GP podem atribuir cargos de liderança.")
        ...
        if text == "👤 Membro":
             ...
             prompt = "Selecione a nova **Área**:"

             keyboard_options = BotData.AREAS

             session['reassign_substep'] = 'area' # Need area then project

        

        elif text == "👑 Diretor":

             prompt = "Selecione a **Área** que será liderada:"

             keyboard_options = BotData.AREAS

             session['reassign_substep'] = 'scope' # Only one step

             

        elif text == "💼 Gerente":

             prompt = "Selecione o **Projeto** que será gerenciado:"

             keyboard_options = BotData.PROJECTS + ["Odoyá", "Maná"] # Ensure all projects available

             # Deduplicate

             keyboard_options = list(set(keyboard_options))

             session['reassign_substep'] = 'scope' # Only one step



        session['step'] = 'reassign_scope'

        session['waiting_for'] = 'reassign_scope'
        ...
        for i in range(0, len(keyboard_options), 2):
            ...
            row.append(KeyboardButton(keyboard_options[i]))

            if i + 1 < len(keyboard_options):

                 row.append(KeyboardButton(keyboard_options[i+1]))
    ...
    async def _handle_reassign_scope(self, update, session, text):

        """Handle scope (Area/Project) selection"""

        # Validate based on substep

        substep = session.get('reassign_substep')

        

        valid_options = BotData.AREAS + BotData.PROJECTS + ["Odoyá", "Maná"]

        if text not in valid_options: # Loose validation

             await update.message.reply_text("❌ Opção não reconhecida. Selecione uma opção válida.")
        ...
        if substep == 'area':
             ...
             # Member flow: User selected Area, now ask Project

             session['reassign_new_area'] = text

             session['reassign_substep'] = 'project'

             

             prompt = f"✅ Área: **{text}**\nAgora selecione o **Projeto**:"

             keyboard_options = BotData.PROJECTS
        ...
        elif substep == 'project':

             # Member flow: User selected Project. Finalize.

             area = session['reassign_new_area']
             ...
             await self._finalize_reassignment(update, session, roles=[area, project], managed_scopes=[])
             ...
             # Leadership flow: User selected Scope. Finalize.

             role_type = session['reassign_new_type']

             

             roles = [] # Role name usually depends on mapping (handled in start_command)

             # But we need to store something in 'roles'. 

             # Director -> Role = Area Name? Or just "Diretor"?

             # Current convention seems to be putting the Area Acronym in Roles and ManagedScopes.

             

             # If Director of GP: Roles=["GP"], Managed=["GP"]

             # If Manager of Odoya: Roles=["Odoya"], Managed=["Odoya"]

             

             scope = text

             await self._finalize_reassignment(update, session, roles=[scope], managed_scopes=[scope])

             return



    async def _finalize_reassignment(self, update, session, roles, managed_scopes):

        """Apply changes to DB and notify"""

        name = session['reassign_target_name']

        telegram_id = session.get('reassign_target_id')

        role_type = session.get('reassign_new_type')

        

        # Get existing email/matricula to preserve them

        # We need to fetch the member object again or use what we have if we trust it didn't change

        # member object in BotData might not have email/matricula populated if load_members didn't run recently?

        # Actually load_members runs on startup.

        

        member = BotData.get_member(telegram_id) if telegram_id else None

        # If telegram_id is None (member exists in name but no ID connected), we rely on name matching in DB update

        

        email = member.email if member else None

        matricula = member.matricula if member else None

        

        # Update Sheets

        # Note: update_member_registration uses Name to find row.

        sheets_manager.update_member_registration(

            member_name=name,

            telegram_id=telegram_id if telegram_id else 0, # If None, maybe 0 or keep empty string logic in sheet manager
        ...
        movement_desc = f"Alterado para {role_type}: {', '.join(roles)}"

        sheets_manager.log_member_movement(name, telegram_id, movement_desc, movement_type="Remanejamento")
        ...
        await update.message.reply_text(
            ...
            f"O membro **{name}** foi remanejado.\n"

            f"Nova Função: **{role_type}**\n"
        ...
        # Offer to reassign another or return to menu

        keyboard = [

            [KeyboardButton("🔄 Remanejar Outro"), KeyboardButton("🏠 Menu Principal")]
        ...
        session['step'] = 'reassign_finish'

        session['waiting_for'] = 'reassign_finish'



    async def _handle_reassign_finish(self, update, session, text):

        """Handle decision after reassignment"""

        if text == "🔄 Remanejar Outro":

            await self._start_reassign_flow(update.message, session)
            ...
            await self._show_main_menu_message(update)
             ...
             await update.message.reply_text("❌ Opção inválida. Selecione uma opção do teclado.")



    async def refresh_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):

        """Handle /refresh command - Refresh DB data from Sheets"""

        user_id = update.effective_user.id

        

        # No permission check requested by user

        # if not BotData.is_user_in_role(user_id, "GP"):

        #      await update.message.reply_text("❌ Acesso negado. Apenas membros de GP podem usar este comando.")

        #      return

             

        await update.message.reply_text("🔄 Atualizando banco de dados... Por favor aguarde.")
        ...
        try:
            ...
            if members:
                ...
                await update.message.reply_text(f"✅ Sucesso! {count} membros carregados do Google Sheets.")

            else:

                await update.message.reply_text("⚠️ Aviso: Nenhum membro encontrado ou erro ao ler a planilha.")

                

            # Reload Reminders (Optional but good)

            # reminders = sheets_manager.load_reminders()

            # if reminders: storage.reminders = reminders
            ...
            await update.message.reply_text("❌ Ocorreu um erro ao atualizar o banco de dados.")



    async def cadastro_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):

        """Handle /cadastro command - Start registration flow"""
        ...
        session['waiting_for'] = 'registration_name'

        

        await update.message.reply_text(

            "📝 **Cadastro de Membro - Enactus**\n\n"
    ...
    async def _handle_registration_name(self, update, session, text):

        """Handle name input during registration with Fuzzy Matching"""

        user_input = text.strip()

        

        # Fuzzy match against all DB members (even those without ID)

        all_names = BotData.MEMBERS_DB

        # Extract names for matching

        candidate_names = [m.name for m in all_names if not m.telegram_id] # Match only unconnected profiles? Or all?

        # Maybe match all, but prioritize unconnected. For simplicity, match all names.

        candidate_names_all = [m.name for m in all_names]

        

        matches = get_close_matches(user_input, candidate_names_all, n=3, cutoff=0.6)
        ...
        if not matches:
             ...
             await update.message.reply_text(
                 ...
                 "Seu nome não consta no nosso Banco de Dados de membros.\n"

                 "Por favor, tente digitar novamente ou contate um membro de **Gestão de Pessoas** caso seja um engano."
        ...
        # If exact match or close match found

        # If multiple matches, we could ask to clarify, but usually name is unique enough.

        # Let's take the best match.

        best_match = matches[0]

        

        # Check if this member is already connected to another ID?

        existing_member = next((m for m in all_names if m.name == best_match), None)

        if existing_member and existing_member.telegram_id and existing_member.telegram_id != update.effective_user.id:

             await update.message.reply_text(

                 f"⚠️ O membro **{best_match}** já está associado a outro usuário do Telegram.\n"

                 "Entre em contato com a diretoria se isso for um erro."
        ...
        session['reg_name'] = best_match

        session['step'] = 'registration_email'

        session['waiting_for'] = 'registration_email'

        

        await update.message.reply_text(

            f"✅ Nome localizado: **{session['reg_name']}**\n\n"

            "Agora, por favor digite seu **Email**:",
    ...
    async def _handle_registration_email(self, update, session, text):

        """Handle email input"""

        if not re.match(r"[^@]+@[^@]+\.[^@]+", text):

            await update.message.reply_text("❌ Email inválido. Por favor, tente novamente.")

            return



        session['reg_email'] = text.strip()

        session['step'] = 'registration_matricula'

        session['waiting_for'] = 'registration_matricula'
    ...
    async def _handle_registration_matricula(self, update, session, text):

        """Handle matricula input"""

        # Format: 7 digits + 3 letters (e.g. 2122030BCC)

        if not re.match(r"^\d{7}[a-zA-Z]{3}$", text.strip()): 

             await update.message.reply_text("❌ Matrícula inválida. Formato esperado: 1234567ABC (7 números seguidos de 3 letras).")

             return

             

        session['reg_matricula'] = text.strip()

        session['step'] = 'registration_area'

        session['waiting_for'] = 'registration_area'
...
Agora, selecione sua **Área** de atuação:
        ...
        for i in range(0, len(BotData.AREAS), 2):

            row = []

            row.append(KeyboardButton(BotData.AREAS[i]))

            if i + 1 < len(BotData.AREAS):

                row.append(KeyboardButton(BotData.AREAS[i+1]))
    ...
    async def _handle_registration_area(self, update, session, text):

        """Handle area selection"""

        if text not in BotData.AREAS:

             await update.message.reply_text("❌ Selecione uma Área válida.")

             return

             

        session['reg_area'] = text

        session['step'] = 'registration_project'

        session['waiting_for'] = 'registration_project'
...
Agora, selecione seu **Projeto**:
        ...
        for i in range(0, len(BotData.PROJECTS), 2):

             row = []

             row.append(KeyboardButton(BotData.PROJECTS[i]))

             if i + 1 < len(BotData.PROJECTS):

                 row.append(KeyboardButton(BotData.PROJECTS[i+1]))
    ...
    async def _handle_registration_project(self, update, session, text):

        """Handle project selection and finalize registration"""

        if text not in BotData.PROJECTS:

              await update.message.reply_text("❌ Selecione um Projeto válido.")

              return

        

        session['reg_project'] = text
        ...
        area = session['reg_area']

        project = session['reg_project']

        email = session.get('reg_email')

        matricula = session.get('reg_matricula')
        ...
        BotData.update_member_locally(name, user_id, roles, managed_scopes=[], email=email, matricula=matricula)

        

        # 2. Update Sheets (DB)

        success_db = sheets_manager.update_member_registration(name, user_id, roles, managed_scopes=[], email=email, matricula=matricula)

        

        # 3. Log History

        success_log = sheets_manager.log_member_movement(name, user_id, f"Registrado em: {area}, {project}")
...
Utilize /start para ver o menu principal.

        """

        

        await update.message.reply_text(final_text, reply_markup=ReplyKeyboardRemove(), parse_mode='Markdown')
...
**Projetos disponíveis:** Odoyá, Maná
    ...
    async def handle_text_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        elif waiting_for == 'registration_name':

            await self._handle_registration_name(update, session, text)

        elif waiting_for == 'registration_email':

             await self._handle_registration_email(update, session, text)

        elif waiting_for == 'registration_matricula':

             await self._handle_registration_matricula(update, session, text)

        elif waiting_for == 'registration_area':

            await self._handle_registration_area(update, session, text)

        elif waiting_for == 'registration_project':

            await self._handle_registration_project(update, session, text)

            

        # Reassignment Flow Steps

        elif waiting_for == 'reassign_member':

            await self._handle_reassign_member_selection(update, session, text)

        elif waiting_for == 'reassign_role_type':

            await self._handle_reassign_role_type(update, session, text)

        elif waiting_for == 'reassign_scope':

             await self._handle_reassign_scope(update, session, text)

        elif waiting_for == 'reassign_finish':

             await self._handle_reassign_finish(update, session, text)
